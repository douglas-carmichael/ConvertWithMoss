...
* Kurzweil K2x00
  * New: The DSP parameters of the program layers are now converted, based on the hardware reverse-engineering of the mpc2emu project: reading gets the layer velocity window, the amplitude envelope (unless the layer plays the 'natural' envelope of its samples), the filter with its cutoff and resonance and the filter envelope or cutoff velocity modulation with its depth; writing stores the global amplitude envelope, filter and filter envelope (or cutoff velocity modulation) of the multi-sample. Envelope stages with a zero time and level are unused on the device and keep the previous level, so e.g. an FM bass program with an unused decay stage sustains at the attack level instead of falling silent.
