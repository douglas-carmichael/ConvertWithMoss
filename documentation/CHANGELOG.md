...
* Fixed: A category tag at the start of a preset name lost against a folder in its path which starts with another one - 'BASS Growler' in a folder named 'Pad Sounds' was detected as Pad - which is exactly what the option 'Category tag at name start declares the category' is meant to prevent. The names are searched in their order again, the preset name before its folders; the collection which holds them had become an unordered one.
...
* Waldorf Quantum/Iridium
  * New: The four attributes of a written preset are now also filled from the words of the preset name which the device uses itself. Several categories of this application map onto the same attribute of the device, so a name which announces a granular or a physically modelled sound - 'GRAN Fields' or 'PHYS Digibow', a wide-spread naming convention - is written with 'Granular' respectively 'Resonator' next to its category 'Synth', which the category alone cannot say.
  * Fixed: Keywords were written as attributes in the lower case spelling of this application, so a 'poly' stood in the filter list of the device between the capitalized words of the factory sound sets. A keyword is now written capitalized and, if the device has a word of its own for it, in that spelling ('noisy' becomes 'Noise', 'spacey' becomes 'Space', 'atmo' becomes 'Atmo').
  * New: A preset which carries no attributes now gets its category and keywords - and its author - from its file name, its folders and its bank, the way every other format does. A library which was renamed or sorted into folders on the computer can therefore be tagged by converting it from Quantum/Iridium to Quantum/Iridium; the attributes which a preset does carry are kept.
  * Fixed: The metadata settings of the source format were stored and addressed under the name of the Korg Wavestate/Modwave format, so both formats shared one set of them and the command line parameters of the Quantum/Iridium had to be spelled with the name of the Korg. They are now named after the format itself (e.g. 'QPATCategoryFromNamePrefix'); the settings have to be set once more.
