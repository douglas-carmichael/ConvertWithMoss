...
* Category tag at name start declares the category: Many commercial libraries prefix each preset name with its category (e.g. 'PAD Solina', 'BASS Growler'). If enabled, such a tag at the very start of the name declares the category and takes precedence over keyword matches elsewhere in the name (e.g. 'BELL Vibrato Strings' is then detected as Bell instead of Strings). Common abbreviations are recognized as well: BRAS, DRM, FLUT, GRAN, ORG, PERC, PHYS, PLUK, POLY, REES, STRG, SWEP, VOC. Leave it off for libraries which do not follow this naming convention (a name like 'Syn Brass' would otherwise be detected as Synth instead of Brass). The preset name is searched before the folders which contain it, so a tag in the name wins over a folder which starts with another one ('BASS Growler' in a folder named 'Pad Sounds' is detected as Bass).
...
The device lists four attributes next to the preset name and filters its library by them. They are written from the category of the source, from those words of the preset name which the device uses itself and from the keywords of the source, in that order, and always in the spelling of the factory sound sets: a *Keyboard* becomes *Keys*, a *Bell* becomes *Bells*, and a kick, a snare, a clap or a hi-hat becomes *Percussive*, so that a converted preset lands in the same filter as the factory content instead of next to it. The name is searched because several categories of this application map onto the same word of the device - a name which announces a granular or a physically modelled sound (e.g. *GRAN Fields* or *PHYS Digibow*) is written with *Granular* respectively *Resonator* next to its category *Synth*. A category which was not detected is left out instead of filling the filter list with the word *Unknown*.

Reading a preset takes its category and keywords from those attributes. A preset which carries none - and one whose file was renamed on the computer - is treated like a file of any other format: the category, the keywords and the author are guessed from the file name, the folders and the bank, which is what the [metadata options](#automatic-metadata-detection) of the source are for. A library on a memory card can therefore be tagged afterwards by renaming its files (e.g. *PAD Solina.qpat*) and converting it from Quantum/Iridium to Quantum/Iridium; the preset name which the device displays is stored in the file and is not touched by such a rename.
